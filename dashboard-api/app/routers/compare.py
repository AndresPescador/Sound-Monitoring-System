...
async def compare_measurements(
    ...
    """
    Compara mediciones usando un grid temporal común y conserva un detalle crudo.

    ``data`` usa los mismos buckets para todas las estaciones, por lo que sus
    líneas sí comparten las posiciones del eje X. ``raw_data`` mantiene los
    timestamps originales para el ScatterChart; puede venir limitado por
    ``raw_limit`` y lo informa por estación.
    """
    ...
    raw_result = await db.execute(
        text(f"""
            ...
            SELECT station_code, locality, recorded_at, value
            FROM ranked
            WHERE row_number <= :raw_limit
            ORDER BY station_code ASC, recorded_at ASC
        """),
        raw_params,
    )
    ...
    for row in raw_result.mappings().all():
        series_map[row["station_code"]]["raw_data"].append({
            "recorded_at": row["recorded_at"],
            "value": row["value"],
        })
        series_map[row["station_code"]]["raw_returned_count"] += 1
